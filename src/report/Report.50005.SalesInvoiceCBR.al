report 50005 "Sales Invoice CBR"
{
    DefaultLayout = RDLC;
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(TrackingID; "Sales Invoice Header"."Package Tracking No.")
                    {
                    }
                    ...
                    dataitem(SalesInvLine; "Integer")
                    {
                        ...
                        column(Discount; TempSalesInvoiceLine."Line Discount Amount")
                        {
                        }
                        ...
                        column(NoofCase; NoofCase)
                        {
                        }
                        column(NoofShipCase; NoShipCase)
                        {
                        }
                        column(TotalCubes; TotalCubes)
                        {
                        }
                        ...
                        begin
                            ...
                            with TempSalesInvoiceLine do begin
                                ...
                                if ItemUnitofMeasure.Get(Item."No.", Item."Base Unit of Measure") then begin
                                    TotalCubes := ItemUnitofMeasure.Cubage;
                                    TotalWeight := ItemUnitofMeasure.Weight;
                                end;
                                if UnitofMeasure.Get(Item."Base Unit of Measure") then
                                    UOMDesc := UnitofMeasure.Description;

                                NoofCase := "No. of Cases";
                                NoShipCase := "No. of Cases Ship";
                                QtyShipped := Quantity;

                                if SalesLine.Get(SalesLine."Document Type"::Order, "Order No.", "Order Line No.") then begin
                                    if SalesLine.Quantity <> Quantity then begin
                                        if SalesLine."Outstanding Quantity" <> 0 then
                                            QtyBO := SalesLine."Outstanding Quantity"
                                        else
                                            QtyBO := SalesLine.Quantity - SalesLine."Quantity Shipped";
                                    end else
                                        QtyOrdered := Quantity;

                                    QtyInvoiced := SalesLine."Quantity Invoiced";
                                end else begin
                                    ...
                                    if SalesLineArchive.FindLast then begin
                                        if SalesLineArchive.Quantity <> Quantity then
                                            QtyOrdered := SalesLineArchive.Quantity
                                        else
                                            QtyOrdered := Quantity;

                                        QtyInvoiced := SalesLineArchive."Quantity Invoiced";
                                    end;
                                end;
                                if QtyInvoiced = 0 then begin
                                    if QtyBO = 0 then
                                        QtyBO := QtyOrdered - QtyShipped;
                                end else begin
                                    QtyBO := QtyOrdered - QtyInvoiced;
                                end;
                        ...
                        begin
                            ...
                            TempSalesInvoiceLine.SetFilter("No.", '<>%1', '');
            ...
            begin
                ...
                if ShippingAgentRec.Get("Sales Invoice Header"."Shipping Agent Code") then
                    ShipVia := ShippingAgentRec.Name;
