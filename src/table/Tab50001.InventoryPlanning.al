table 50001 "Inventory Planning"
{
    ...
    fields
    {
        ...
        {
            CalcFormula = Sum("Transfer Line"."Qty. in Transit (Base)" WHERE("Derived From Line No."=CONST(0), "Item No."=FIELD("Item No."), "Transfer-to Code"=FIELD("Location Code")));
        ...
        field(16;"Qty. on Purch. Order";Decimal)
        {
            ...
            CalcFormula = Sum("Purchase Line"."Outstanding Qty. (Base)" WHERE("Document Type"=CONST(Order), Type=CONST(Item), "No."=FIELD("Item No."), "Location Code"=FIELD("Location Code")));
        ...
        field(18;"Qty. on Sales Order";Decimal)
        {
            ...
            CalcFormula = Sum("Sales Line"."Outstanding Qty. (Base)" WHERE("Document Type"=CONST(Order), Type=CONST(Item), "No."=FIELD("Item No."), "Location Code"=FIELD("Location Code")));
    ...
    procedure UpdateInventoryValues()var myInt: Integer;
    recItem: Record Item;
    recLocation: Record Location;
    begin
        if recItem.FindSet()then repeat if recLocation.FindSet()then repeat Rec.Init();
    ...
    local procedure UpdateUOM(UOM: Code[20]): Text[50]var myInt: Integer;
    recUnitMeasure: Record "Unit of Measure";
    begin
        if recUnitMeasure.Get(UOM)then exit(recUnitMeasure.Description);
    end;
    local procedure UpdateUnitCost(ItemNo: code[20];
    Location: Code[20]): Decimal var StockkeepingUnit: Record "Stockkeeping Unit";
    begin
        StockkeepingUnit.Reset();
        StockkeepingUnit.SetRange("Item No.", ItemNo);
        StockkeepingUnit.SetRange("Location Code", Location);
        if StockkeepingUnit.FindFirst()then exit("Unit Cost");
    end;
}
