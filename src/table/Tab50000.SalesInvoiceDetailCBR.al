table 50000 "Sales Invoice Detail CBR"
{
    ...
    var procedure UpdateItemHistory()var SalesInvHead: Record "Sales Invoice Header";
    SalesInvLine: Record "Sales Invoice Line";
    SalesInvoiceDetail: Record "Sales Invoice Detail CBR";
    begin
        if not Confirm('Do you want to refresh data into Item Sales History Table, this action will delete all of the existing sales history lines and re-populate the data from posted Sales Invoice', false)then exit;
        SalesInvoiceDetail.DeleteAll;
        SalesInvHead.Reset;
        if SalesInvHead.Find('-')then repeat if not IFItemSalesLinesExit(SalesInvHead)then begin
                    SalesInvLine.Reset;
                    SalesInvLine.SetRange("Document No.", SalesInvHead."No.");
                    SalesInvLine.SetRange(Type, SalesInvLine.Type::Item);
                    if SalesInvLine.FindSet then repeat CreateLines(SalesInvHead, SalesInvLine);
                        until SalesInvLine.Next = 0;
                end;
            until SalesInvHead.Next = 0;
        Message('Completed Successfully');
    end;
    procedure CreateLines(SInvHead: Record "Sales Invoice Header";
    SInvLine: Record "Sales Invoice Line")var SalesInvoiceDetail: Record "Sales Invoice Detail CBR";
    Item: Record Item;
    ...
    begin
        ...
        if Item.Get(SInvLine."No.")then SalesInvoiceDetail.Collection:=Item."Item Category Code";
        ...
        if recCustomer.Get(SInvHead."Sell-to Customer No.")then SalesInvoiceDetail."Customer Price Group":=recCustomer."Customer Price Group";
    ...
    procedure IFItemSalesLinesExit(SalesInvHead: Record "Sales Invoice Header"): Boolean var SalesInvoiceDetail: Record "Sales Invoice Detail CBR";
    begin
        SalesInvoiceDetail.Reset;
        SalesInvoiceDetail.SetRange("Invoice No", SalesInvHead."No.");
        if SalesInvoiceDetail.Count > 0 then exit(true)
        else
            exit(false)end;
    procedure GetAdjustedCost(InvoiceDocNo: Code[20];
    InvoiceLineNo: Integer): Decimal var recSalesInvoiceLine: Record "Sales Invoice Line";
    ...
    procedure GetPaymentDate(DocNo: Code[20]): Date var CLEInvoice: Record "Cust. Ledger Entry";
    ...
    begin
        ...
        if CLEInvoice.FindFirst()then begin
            if CLEPayment.get(CLEInvoice."Closed by Entry No.")then exit(CLEPayment."Posting Date");
        end;
    end;
}
