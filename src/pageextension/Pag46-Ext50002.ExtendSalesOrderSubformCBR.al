pageextension 50002 "ExtendSalesOrderSubform_CBR" extends "Sales Order Subform" //46
{
    layout
    {
        modify(Quantity)
        {
        trigger OnAfterValidate()var RecItem: Record Item;
        CaseQty: Decimal;
        begin
            clear(CaseQty);
            If RecItem.Get(Rec."No.")then begin
                if RecItem."Case Pack" <> 0 then begin
                    ...
                    iF(CaseQty MOD 1) <> 0 then Error('The Quantity is out of case pack. Please correct. ')
                    else
        ...
        modify("Qty. to Ship")
        {
        trigger OnAfterValidate()var RecItem1: Record Item;
        CaseQty1: Decimal;
        begin
            clear(CaseQty1);
            If RecItem1.Get(Rec."No.")then begin
                if RecItem1."Case Pack" <> 0 then begin
                    ...
                    iF(CaseQty1 MOD 1) <> 0 then Error('The Quantity to Ship is out of case pack. Please correct. ')
                    else
