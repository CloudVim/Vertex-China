page 50005 "NJ Shipping Instructions"
{
    ...
    SourceTableView = SORTING("Document Type", "Document No.", "Line No.")ORDER(Ascending)WHERE(Status=CONST(Released), "Location Code"=CONST('888'), "Qty. to Ship"=FILTER(>0));
    ...
    trigger OnAfterGetRecord()begin
        if recSalesHeader.Get(Rec."Document Type"::Order, Rec."Document No.")then begin
            ...
            if recSalesHeader."Shipper Acct No." <> '' then ShipAccount:=recSalesHeader."Shipper Acct No."
            else
                ShipAccount:='02A1V8';
            AccountZip:='07072';
        end;
    end;
    var recSalesHeader: Record "Sales Header";
    UseInsurance: Text;
    SavedElements: Text;
